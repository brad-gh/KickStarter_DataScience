...
object Preprocessor {
  ...
  def main(args: Array[String]): Unit = {
    ...
    dfCasted.groupBy("final_status").count.orderBy($"count".desc).show //On remarque la presence de valeurs autres que 0 et 1. Cela s'explique par le décalage des cellules dans le tableau
    ...
    // d) On suppose qu'on ne sait pas traiter le texte, donc on concatene toutes les variables texte dans une seule.
    val dfText= dfDurations
      .withColumn("text", concat_ws(" ", $"name", $"desc", $"keywords"))

    /** VALEUR NULLES: traitement des valeurs nulles existantes en leur donnant une valeur extreme. **/
